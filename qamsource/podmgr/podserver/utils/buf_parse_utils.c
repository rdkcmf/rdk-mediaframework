...
void vlTraceStack(int nLevel, VL_TRACE_STACK_TAG_TYPES nTagType, const char * lpszTag, const char * lpszFormat, ...)
{
    ...
    static char m_strStackTraceBuffer[STACK_TRACE_MAX_DEPTH*STACK_TRACE_INDENT + STACK_TRACE_MAX_MSG + 20];
    static char m_strStackTraceTag[STACK_TRACE_MAX_MSG+10];
    unsigned int uiWriteBuffLen = 0;
    ...
    if((VL_TRACE_STACK_TAG_END != nTagType) && (nLevel >= 0) && (nLevel < STACK_TRACE_MAX_DEPTH))
    {
        ...
        {
            /* open the tag */
            uiWriteBuffLen = sizeof(m_strStackTraceBuffer) - (nLevel*STACK_TRACE_INDENT);
            if(uiWriteBuffLen > 0)
            {
                uiWriteBuffLen -= snprintf(m_strStackTraceBuffer + nLevel*STACK_TRACE_INDENT, uiWriteBuffLen, "{-->> %s ", m_strStackTraceTag);
            }
            else
            {
                 RDK_LOG(RDK_LOG_INFO, "LOG.RDK.POD", "no buffer 2 write the tag %d", uiWriteBuffLen);
            }
        }
    ...
    if((VL_TRACE_STACK_TAG_END == nTagType) && (nLevel >= 0) && (nLevel < STACK_TRACE_MAX_DEPTH))
    {
        ...
        if(VL_TRACE_STACK_TAG_END == nTagType)
        {
            ...
            if(uiWriteBuffLen > 0)
            {
                 uiWriteBuffLen -= snprintf(m_strStackTraceBuffer + nLevel*STACK_TRACE_INDENT, uiWriteBuffLen, "}<<-- %s\n", m_strStackTraceTag);
            }
            else
            {
                RDK_LOG(RDK_LOG_INFO, "LOG.RDK.POD", "writing tag failed [%d]", uiWriteBuffLen);
            }
        }
        /* print the buffer */
        RDK_LOG(RDK_LOG_INFO, "LOG.RDK.POD", m_strStackTraceBuffer);
